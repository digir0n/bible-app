...
function formatSearchReference(row) {
    const verseNumber = Math.round(
        (Number(row.verse) - Math.floor(Number(row.verse))) * 1000
    );

    const chapter = Math.floor(Number(row.verse));
...
function renderSearchResults(rows) {
    ...
    if (!rows.length) {
        elements.searchStatus.textContent = "Geen resultate gevind nie.";
        return;
    }
    ...
    for (const row of rows) {
        ...
        text.textContent = row.unformatted;
        ...
        button.addEventListener("click", async () => {
            try {
                ...
                reader.scrollToVerse(reference.verse);
...
async function performSearch() {
    ...
    try {
        const rows = state.database.searchVerses(term, SEARCH_LIMIT);
